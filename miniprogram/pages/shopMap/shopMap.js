...
Page({
  ...
  onLoad: function () {
    ...
    db.collection('register').where({userName:withlovename}).get({
  ...
  addMarker: function (data) {
    ...
    mks.push({ // 获取返回结果，放到mks数组中
      ...
      iconPath: "../../images/biaodiandidian.png", //图标路径
      width: 25,
      height: 25
    })
...
db.collection('Map').add({
  data: {
    ...
    address:this.data.centerData.addr,
...
// 查询用户 
  ...
  updatamap(id){
    console.log("修改")
    console.log(id);
    // var  arr=this.data.centerData.addr
    console.log(this.data.centerData.addr);
    console.log(this.data.centerData.latitude);
    console.log( this.data.centerData.longitude);
   
    const dbd = wx.cloud.database();
    dbd.collection('Map').doc(
              id
    ).update({
      data:{
        latitude:this.data.centerData.latitude,
